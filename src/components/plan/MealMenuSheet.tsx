...
const MEALS: MealSlot[] = ['almoco', 'jantar'];
...
export function MealMenuSheet({
  ...
  const { t } = useLanguage();
  const MEAL_LABEL: Partial<Record<MealSlot, string>> = {
    almoco: t('plan.lunch'),
    ...
    <BottomSheet open={open} onClose={close}>
      {view === 'menu' ? (
        <div className="flex flex-col gap-1">
          {recipe ? (
            <Action
              label={t('mealMenu.viewRecipe')}
              onClick={() => void navigate(routes.recipe(recipe.slug))}
            />
          ) : null}
          {entry.entry_type !== 'leftover' ? (
            <Action
              label={t('mealMenu.changeRecipe')}
              onClick={() => {
                close();
                onTrocarReceita();
              }}
          ...
          <Action label={t('mealMenu.moveToOtherDay')} onClick={() => setView('move')} />
          {isRecipeLike ? (
            <Action label={t('mealMenu.changeServings')} onClick={() => setView('servings')} />
          ) : null}
          {entry.entry_type === 'recipe' && tomorrowIsFree ? (
            <Action label={t('mealMenu.createLeftoverTomorrow')} onClick={createLeftoverTomorrow} />
          ) : null}
          <Action
            icon={entry.locked ? Unlock : Lock}
            label={entry.locked ? t('mealMenu.unlock') : t('mealMenu.lockChoice')}
            onClick={() => setLocked.mutate({ id: entry.id, locked: !entry.locked }, { onSuccess: close })}
          />
          {isRecipeLike ? (
            <Action
              label={entry.status === 'cooked' ? t('mealMenu.undoCooked') : t('mealMenu.alreadyAte')}
              onClick={() =>
          ...
          <Action
            tone="danger"
            label={t('mealMenu.removeFromWeek')}
      ...
      {view === 'move' ? (
        <div className="flex flex-col gap-5">
          <div>
            <p className="mb-2 text-small text-ink-muted">{t('mealMenu.day')}</p>
          ...
          <div>
            <p className="mb-2 text-small text-ink-muted">{t('mealMenu.meal')}</p>
          ...
          {moveEntry.isError ? (
            <p className="text-small text-rouge">{t('mealMenu.slotTaken')}</p>
          ) : null}
        </div>
      ) : null}
      ...
      {view === 'servings' ? (
        <div className="flex flex-col items-center gap-4">
          <div className="flex items-center gap-5">
            <button
              type="button"
              aria-label={t('mealMenu.fewerServings')}
              disabled={servings <= 1}
            ...
            <button
              type="button"
              aria-label={t('mealMenu.moreServings')}
